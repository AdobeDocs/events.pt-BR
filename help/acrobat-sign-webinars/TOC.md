---
user-guide-title: Webinários do Acrobat Sign
user-guide-description: Aprenda com especialistas da Adobe como dominar novos fluxos de trabalho de assinatura eletrônica no Acrobat Sign.
cloud: Document Cloud
solution: Document Cloud
source-git-commit: 23f0005edc68e39508f2435c5e633e9eb82456de
workflow-type: tm+mt
source-wordcount: '121'
ht-degree: 5%

---


# Webinários do Acrobat Sign {#acrobat-sign-webinars}

+ [Visão geral](overview.md)
+ Webinários de lançamento {#releases}
   + [Fevereiro de 2026](skill-builders/acrobat-sign-february-2026-release.md)
   + [Outubro de 2025](skill-builders/acrobat-sign-october-2025-release.md)
   + [Julho de 2025](skill-builders/acrobat-july-release.md)
+ Inovação - destaques dos novos recursos {#newfeatures}
   + [Introdução aos fluxos de trabalho personalizados](./skill-builders/custom-workflows.md)
   + [Verificação e segurança de identidade](./skill-builders/identity-verification-security.md)
   + [Otimizar assinatura eletrônica móvel](./skill-builders/mobile-e-signing.md)
+ 101 - Introdução para usuários {#getstartedusers}
   + [Acrobat Sign 101](./skill-builders/getting-started.md)
   + [Ambiente de criação](./skill-builders/authoring-environment.md)
   + [Preparar seus Contratos para assinatura eletrônica](./skill-builders/e-signature-setup.md)
   + [Visibilidade do Status do Contrato](./skill-builders/agreement-status.md)
   + [Trabalhar com assinaturas eletrônicas em aplicativos Microsoft](./skill-builders/e-signature-microsoft.md)
   + [Criação e uso de workflows](./skill-builders/create-use-workflows.md)
+ 101 - Introdução para administradores {#getstartedadmins}
   + [Acelere sua experiência de administrador do Acrobat Sign](./skill-builders/acrobat-sign-admin-experience.md)
   + [Administradores no ETLA](./skill-builders/acrobat-sign-admin-etla.md)
   + [Relatórios avançados](./skill-builders/advanced-reporting.md)
+ 102 - Tarefas avançadas {#advancedtasks}
   + [Dicas avançadas para enviar documentos para assinatura](./skill-builders/advanced-sending-documents-signature.md)
   + [Colete muitas assinaturas com um clique](./skill-builders/collect-signatures.md)
   + [Document Builder](./skill-builders/document-builder.md)
   + [Publicar um formulário Web preenchível e assinável](./skill-builders/fillable-signable-web-form.md)
   + [Automações de Fluxo de Trabalho](./skill-builders/workflow-automations.md)
