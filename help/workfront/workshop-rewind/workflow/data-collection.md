---
title: Dicas do Workfront Pro - Oportunidades de coleta de dados de próximo nível na Workfront
description: Explore técnicas avançadas para otimizar a coleta de dados no Workfront, melhorando a eficiência e a governança.
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3546
last-substantial-update: 2026-03-26T00:00:00Z
jira: KT-20603
source-git-commit: c350251d880f9e1ea4de6de51e1139894c9a3b47
workflow-type: tm+mt
source-wordcount: '175'
ht-degree: 2%

---


# Dicas do Workfront Pro - Oportunidades de coleta de dados de próximo nível na Workfront

Hospedado em parceria com Steve Teitelbaum, arquiteto corporativo do Content Supply chain na Adobe, saiba como recursos avançados em formulários personalizados e regras de negócios do Workfront podem fornecer mais controle sobre como os dados são coletados e o que isso significa para simplificar operações, relatórios, integrações do Workfront e casos de uso de IA.

>[!VIDEO](https://video.tv.adobe.com/v/3481821/?learn=on&enablevpops)

Esta sessão amplia a discussão de um workshop anterior, [Workfront Pro Tips for Governance, Execution, and Adoption](https://experienceleague.adobe.com/pt-br/docs/events/workfront-recordings/workshop-rewind/workflow/governance-execution-adoption).

Se você perdeu a sessão, não se preocupe! Revise a [apresentação de slides](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/Workfront+Pro+Tips+Part+2_Dec2025.pdf) e assista à gravação sob demanda para acompanhá-la.

Novos eventos são adicionados a cada mês; portanto, verifique a [página Eventos da Experience League](https://experienceleague.adobe.com/pt-br/events) para obter as sessões mais recentes.

